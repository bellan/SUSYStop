...
namespace smooth{
  ...
  bool isEmpty (TH2F* h, int ix, int iy) {
...
//
TH2F* findLimitHisto (TFile* file, std::string hname) {
  // result == 0 : failure
  TH2F* result(0);
  //
...
void fillTriplets (std::vector<BinContentTriplet>& triplets, TH2F* h, int nbx, int nby,
...
TH2F* fitMissing (TH2F* h) {
  HEffErr = (TH2F*)h->Clone("HEffErr");
  HEffErr->SetTitle("EffErr");
  HEffErr->Reset();
  HEffWidth = (TH2F*)h->Clone("HEffWidth");
  HEffWidth->SetTitle("EffWidth");
  HEffWidth->Reset();

  TH2F* hNew = (TH2F*)h->Clone("hSmoothFit");
  hNew->Reset();
...
//
TH2F* fillMissing (TH2F* h, int nmin=5, int nmin2=14, bool useLog = false) {
  //
  // prepare histogram
  //
  TH2F* hNew = (TH2F*)h->Clone("hFilled");
  ...
  for ( int ix=1; ix<=nbx; ++ix ) {
    for ( int iy=1; iy<=nby; ++iy ) {
      ...
      if ( !smooth::isEmpty(h,ix,iy) )  continue;
      // clear matrix and vector used for fit
      int nn(0);
      for ( int i=0; i<3; ++i ) {
	cvec(i) = 0.;
...
//
void clearBins (TH2F* histo, TH2F* refHisto) {
  int nbx = refHisto->GetNbinsX();
  int nby = refHisto->GetNbinsY();
  // loop over bins
  for ( int ix=1; ix<=nbx; ++ix ) {
    for ( int iy=1; iy<=nby; ++iy ) {
      if ( smooth::isEmpty(refHisto,ix,iy) )
...
// //    the physical region)
// //
// void clearBins (TH2F* histo, TH2F* refHisto, int nmax = 1) {
//   int nbx = refHisto->GetNbinsX();
//   int nby = refHisto->GetNbinsY();
//   // loop over bins
//   for ( int ix=1; ix<=nbx; ++ix ) {
...
//
TH2F* doSmoothEff (TH2F* hRaw, bool ratio = true, bool draw = false) {
  //
  // fill isolated empty bins
  //
  TH2F* hFilled = fillMissing(hRaw);
//   TH2F* hFilledLoose = fillMissing(hRaw,4,6);
...
//   }
  ...
  // smooth histogram 
  //
  TH2F* hSmooth = fitMissing(hFilled);
  delete hFilled;
...
//   }
  ...
  //
//   if ( draw ) {
//     TH2F* hRelDiff = (TH2F*)hFilled->Clone("hRelDiff");
...
//   //
  TH2F* hRatio(0);
//   if ( draw || ratio ) {
//     hRatio = (TH2F*)hSmooth->Clone("hRatio");
...
//   ratio:  if true return ratio smoothed / raw; else smoothed
//
TH2F* doSmooth (TH2F* hRaw, const char* algo = "k3a", int nTimes = 2, bool ratio = true, bool draw = false, bool useLog = false) {
  //
  // fill isolated empty bins
  //
  TH2F* hFilled = fillMissing(hRaw,5,14,useLog);
  TH2F* hFilledLoose = fillMissing(hRaw,4,6,useLog);
  //
...
//     hFilled->Draw("same box");
    TH2F* hTmp = (TH2F*)hFilledLoose->Clone("hTmp");
    for ( int ix=1; ix<=hTmp->GetNbinsX(); ++ix ) {
      for ( int iy=1; iy<=hTmp->GetNbinsY(); ++iy ) {
	if ( !smooth::isEmpty(hFilled,ix,iy) )  hTmp->SetBinContent(ix,iy,0.);
      }
    }
    if ( useLog ) {
...
//     hRaw->Draw("same box");
  ...
  // smooth histogram 
  //
  TH2F* hSmooth = (TH2F*)hFilledLoose->Clone("hSmooth");
  hSmooth->SetTitle("hSmooth");
...
//
TH2F* doEffFit (TH2F* hEff, TH2F* hEvt, bool draw = false) {
  TH2F* hEffFit = doSmoothEff(hEff,false);
  TH2F* hXsec = (TH2F*)hEvt->Clone("hXsec");
  hXsec->Divide(hEff);

  if ( draw ) {
    cout << "In draw" << endl;
    TH2F* hEffFitDraw = (TH2F*)hEffFit->Clone("hEffFitDraw");
    TH2F* hEffRatio = (TH2F*)hEff->Clone("hEffRatio");
...
//   TH2F* hXsecFilled = fillMissing(hXsec,5,14,true);
  TH2F* hXsecFilled = fillMissing(hXsec,4,6,true);
...
// default settings for smoothing efficiency histograms
//
TH2F* doEff (TH2F* hRaw, bool ratio = true, bool draw = false) {
  return doSmooth(hRaw,"k3a",2,ratio,draw);
}
//
// default settings for smoothing JES signal systematics
//
TH2F* doJES (TH2F* histo, bool ratio = true, bool draw = false) {
  return doSmooth(histo,"",2,ratio,draw);
}
TH2F* doJES (TFile* file, bool ratio = true, bool draw = false) {
  TH2F* hRaw = findJesHisto(file);
...
//
TH2F* doLimits (TFile* file, string name = "hObs", bool ratio = true, bool draw = false) {
  TH2F* hRaw = findLimitHisto(file,name);
  if ( hRaw == 0 )  return hRaw;
