...
inline bool isEmpty (TH2F* h, int ix, int iy) {
...
//   surrounding bins 
//
void fitQuadratic (TH2F* h, TH2F* refHisto, int nmin=9, int nmin2=12) {
  //
  // prepare histogram
  //
  TH2F* hOrig = (TH2F*)h->Clone();
  ...
  for ( int ix=1; ix<=nbx; ++ix ) {
	...
	for ( int jy=-1; jy<2; ++jy ) {
	  ...
	  if ( isEmpty(refHisto,ix+jx,iy+jy) ) continue;
...
//   surrounding bins 
//
void fitLinear (TH2F* h, TH2F* refHisto, int nmin=5, int nmin2=14) {
  //
  // prepare histogram
  //
  TH2F* hOrig = (TH2F*)h->Clone();
...
TH2F* doSmooth (TH2F* hRaw, bool useLog = true) {
  //
  // smooth histogram 
  //
  TH2F* hSmooth = (TH2F*)hRaw->Clone("hSmooth");
